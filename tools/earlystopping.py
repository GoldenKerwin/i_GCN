...
class EarlyStopping:
    ...
    def __call__(self, val_loss, accs,F1,F2,F3,F4,model,modelname,str):
        ...
        score = -val_loss
        #得分等于验证损失的负值。因为我们希望最小化验证损失，所以得分越高越好。
        ...
        if self.best_score is None:
            ...
            #如果 self.best_score 是 None，那么这是第一次迭代，我们将当前得分设为最佳得分，并保存当前的模型。
            ...
            #如果当前得分低于最佳得分，那么我们增加计数器。如果计数器达到我们设定的耐心值，那么我们将 self.early_stop 设为 True，表示需要提前停止训练
        else:
            self.best_score = score
            self.accs = accs
            self.F1 = F1
            ...
            self.F4 = F4
            self.save_checkpoint(val_loss, model,modelname,str)
            #如果当前得分高于最佳得分，那么我们更新最佳得分，并保存当前的模型。
            self.counter = 0
    ...
    def save_checkpoint(self, val_loss, model,modelname,str):
        ...
        #将模型的状态字典保存到一个文件中，文件名由 modelname、str 和扩展名 .m 组成。
